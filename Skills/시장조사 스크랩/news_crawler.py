...
# 저장: 리서치 애널리스트 하위 '시장동향' 폴더 (using logical name)
MARKET_RESEARCH_FOLDER = get_file_path("market_research_folder", must_exist=True)
OUTPUT_SUBFOLDER = "10. 전략팀/리서치 애널리스트/시장동향"  # Fallback if logical name not found
SUBFOLDER_NAME = "시장동향"  # 리서치 애널리스트 하위 폴더명
...
def main() -> None:
    ...
    if MARKET_RESEARCH_FOLDER and MARKET_RESEARCH_FOLDER.exists():
        output_dir = MARKET_RESEARCH_FOLDER / SUBFOLDER_NAME / date_folder
    else:
