...
```bash
python3 "00. AR/todo_from_today.py"
```
...
### 기본 사용 (오늘 날짜 파일에 추가)
```bash
python3 "00. AR/create_todo.py" "할 일 내용"
```

### 특정 파일에 추가
```bash
python3 "00. AR/create_todo.py" "할 일 내용" --file "00. Todo/Actual/2025-02-15.md"
```

### 완료된 할일로 추가
```bash
python3 "00. AR/create_todo.py" "완료된 작업" --checked
```

### 옵션 조합
```bash
python3 "00. AR/create_todo.py" "할 일 내용" --file "경로/파일.md" --checked
```
...
python3 "Skills/10. Strategy/news_crawler.py"
```

### 한 줄 실행 (어느 디렉토리에서든)
```bash
cd /Users/churryboy/Desktop/proby-sync && python3 "Skills/10. Strategy/news_crawler.py"
```
...
```bash
cd /Users/churryboy/Desktop/proby-sync
python3 "Skills/10. Strategy/social_listener.py"
```

### 커스텀 키워드 (쉼표 구분)
```bash
python3 "Skills/10. Strategy/social_listener.py" "Proby,user research,market research"
```
...
```bash
python3 youtube_to_md.py "https://www.youtube.com/watch?v=4uzGDAoNOZc"
```

### 출력 파일명을 직접 지정
```bash
python3 youtube_to_md.py "https://www.youtube.com/watch?v=4uzGDAoNOZc" "my_video_note.md"
```
